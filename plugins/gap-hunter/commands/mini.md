---
description: The mini version of the main event — reduced 3-agent chain for when scope isn't fixed yet.
...
2. Recommend the user runs `/gap-hunter:honestfilter` first if they have not. Allow override.
...
- Do not run all three breadth agents (Methodology, Orchestration, Structure) — that is `go` (the main event), not `mini`.
